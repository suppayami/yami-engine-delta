...
//=============================================================================
 /*:
 * @plugindesc (Requires YEP_EquipBattleSkills.js) Places equippable
 ...
 * ============================================================================
 *
 * This plugin requires YEP_EquipBattleSkills.
 * Make sure this plugin is located under YEP_EquipBattleSkills in the
 * plugin list.
 *
...
DataManager.processESTierNotetags1 = function(group) {
    ...
    obj.equipTier = 1;
		...
		for (var i = 0; i < notedata.length; i++) {
			var line = notedata[i];
			if (line.match(note1)) {
				obj.equipTier = parseInt(RegExp.$1).clamp(1, 6);
			}
		}
	}
};
...
Game_Actor.prototype.equipSkill = function(skillId, slotId) {
    if (this._learningSkill) {
      var tier = $dataSkills[skillId].equipTier;
      var cur = this.getEquipSkillTierCount(tier);
      var max = this.getEquipSkillTierMax(tier);
      if (cur >= max) return;
    }
...
Window_SkillEquip.prototype.drawSkillTierIcon = function(skill, rect) {
    ...
    this.drawIcon(icon, rect.width - 4 - Window_Base._iconWidth, rect.y);
};
