 ...
 * calculation with more features, effects, and control.
 ...
 * much control for everything else after calculating the damage formula. This
 * plugin will give you control over the order the damage fomrula is calculated
 * in addition to letting you insert your own changes to the damage formula at
 * whatever you you wish.
 *                                                                  .
...
DataManager.isDatabaseLoaded = function() {
    ...
    this.processDMGNotetags($dataSkills);
    this.processDMGNotetags($dataItems);
		return true;
};

DataManager.processDMGNotetags = function(group) {
...
Game_Action.prototype.evalDamageFormula = function(target) {
    try {
        ...
        if (item.damage.custom) {
          Math.max(eval(item.damage.formula), 0) * sign;
        } else {
