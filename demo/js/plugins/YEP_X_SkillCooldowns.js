 ...
 * skill library with the matching Skill Type to be on cooldown. All other
...
Game_BattlerBase.prototype.startWarmups = function() {
    if (this._warmupTurns === undefined) this.clearWarmups();
		for (var i = 0; i < this.skills().length; ++i) {
			...
			value *= this.cooldownDuration(skill);
			this.setWarmup(skill.id, value);
		}
};
...
Game_BattlerBase.prototype.payCooldownCost = function(skill) {
		for (var skillId in skill.cooldown) {
			...
			var value = skill.cooldown[skillId];
			value *= this.cooldownDuration(skill);
			value = Math.max(value, this.cooldown(skillId));
			this.setCooldown(skillId, value);
		}
};
