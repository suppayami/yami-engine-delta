 ...
 * @desc The amount of columns to be displayed by the command window.
 * Default: 1
 * @default 1
 *
 * @param Command Rows
 * @desc This is the number of visible rows for the command window. .
 ...
 * @desc This is the function activated by this menu command. This is
 * an eval.
 * @default
 *
 * @param Menu 1 Actor Bind
 * @desc If the menu command leads to selecting an actor, this is the
 * function activated after selecting an actor.
 * @default
 *
 * @param ---Menu 2---
 * @default
 ...
 * difference between showing a command and enabling a command is that a command
 * can show, but it can't be selected because it isn't enabled. If you wish for
 * this command to always be enabled, use 'true' without the quotes.
 *
 * Ext
 ...
 * straight from the main menu. The function that is to be bound to this command
 * needs to be accessible from Scene_Menu is some way or another. For commands
 * that are meant to select an actor first, use 'this.commandItem.bind(this)'
 * without the quotes.
 *
 * Actor Bind
 ...
 * ============================================================================
 *
 * The following are some examples to help you add/alter/change the way commands
 * appear for your main menu.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 ...
 * the command name will appear. It draws the name information from the database
 * Text Manager entry for 'Item' and uses whatever you put into the database in
 * here. The symbol 'item' is used to make the item command's unique identifier.
 * In order for the command to show, it will run a 'needsCommand' function to
 * check if it will appear. This 'needsCommand' function is related to your
 * database on whether or not you want the item to appear there. In order for
 * this command to be enabled, it will check for whether or not the main
 * commands are enabled, which is related to whether or not there are actors in
 * the current party. And finally, 'this.commandItem.bind(this)' is the command
 * that will run once the item entry is selected.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 ...
 *
 * The skill command is made using the above example. 'TextManager.skill' is how
 * the command name will appear. It draws the name information from the database
 * Text Manager entry for 'Skill' and uses whatever you put into the database in
 * here. The symbol 'skill' is used to make the skill command's unique
 * identifier. In order for the command to show, it will run a 'needsCommand'
 * function to check if it will appear. This 'needsCommand' function is related
 * to your database on whether or not you want the skill option to appear there.
 * In order for this command to be enabled, it will check for whether or not the
 * main commands are enabled, which is related to whether or not there are
 * actors in the current party. This time, the main bind command is to send the
 * player to the actor selection process using 'this.commandPersonal.bind(this)'
 * instead. Once the player selects an actor, 'SceneManager.push(Scene_Skill)'
 * is then ran to send the player to Scene_Skill to manage the actor's skills.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 *       Name: 'Common Event 1'
