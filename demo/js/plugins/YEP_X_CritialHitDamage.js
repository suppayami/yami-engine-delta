//=============================================================================
// Yanfly Engine Plugins - Damage Extension - Critical Hit
// YEP_X_CritialHitDamage.js
// Version: 1.00
//=============================================================================

var Imported = Imported || {};
Imported.YEP_X_CritialHitDamage = true;
...
//=============================================================================
 /*:
 * @plugindesc (Requires YEP_DamageCore.js) Control over critical hits have
 * been added.
 ...
 * @desc Add a little bonus to your critical hits with flat increases
 * in damage and/or healing.
 ...
 *   hit rate bonuses the user may have an ignoring any critical hit evasion
 ...
 *   any critical hit rate bonuses the user may have an ignoring any critical
 ...
 *   <Certain Hit Critical Rate: -x%>
 *   Alters the critical hit critical rate chance of certain hit skills for the
 *   user by +x% or -x% if this notetag exists in the actor, class, enemy,
 *   weapon, armor, or state notetags. This is an additive trait.
 *
...
Yanfly.Parameters = PluginManager.parameters('YEP_X_CritialHitDamage');
    ...
    obj.critMult = Yanfly.Param.critMult;
		...
		for (var i = 0; i < notedata.length; i++) {
			...
			if (line.match(/<(?:CRITICAL RATE):[ ](\d+)([%％])>/i)) {
				...
				console.log(obj.flatCrit);
				obj.damage.critical = true;
			} else if (line.match(/<(?:CRITICAL RATE FORMULA)>/i)) {
				obj.formulaMode = 'critical rate';
				obj.critRate = '';
				obj.damage.critical = true;
			} else if (line.match(/<\/(?:CRITICAL RATE FORMULA)>/i)) {
				obj.formulaMode = '';
		  } else if (line.match(/<(?:CRITICAL MULTIPLIER FORMULA)>/i)) {
				obj.formulaMode = 'critical multiplier';
				obj.critMult = '';
				obj.damage.critical = true;
			} else if (line.match(/<\/(?:CRITICAL MULTIPLIER FORMULA)>/i)) {
				obj.formulaMode = '';
			} else if (line.match(/<(?:FLAT CRITICAL FORMULA)>/i)) {
				obj.formulaMode = 'flat critical';
				obj.flatCrit = '';
				obj.damage.critical = true;
			} else if (line.match(/<\/(?:FLAT CRITICAL FORMULA)>/i)) {
				obj.formulaMode = '';
			} else {
				if (obj.formulaMode === 'critical rate') {
					obj.critRate = obj.critMult + line + ' ';
				} else if (obj.formulaMode === 'critical multiplier') {
					obj.critMult = obj.critMult + line + ' ';
				} else if (obj.formulaMode === 'flat critical') {
